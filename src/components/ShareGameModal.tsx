...
export function ShareGameModal({ game, onClose }: ShareGameModalProps) {
  ...
  return (
    <div
      ref={backdropRef}
      className="fixed inset-0 z-50 flex items-center justify-center bg-theme-panel/70 backdrop-blur-sm"
      onClick={(e) => {
    ...
    >
      <div
        ...
        className="relative mx-4 flex w-full max-w-md flex-col overflow-hidden rounded-2xl border border-theme-glass bg-theme-panel shadow-[0_24px_60px_rgba(0,0,0,0.6)] ring-1 ring-black/40"
        style={{ maxHeight: "85vh" }}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-theme-glass px-5 py-4">
          <h3 className="text-lg font-semibold text-theme-foreground">
            {t("shareGameModal.title")}
          </h3>
          <button
            onClick={onClose}
            className="inline-flex items-center justify-center rounded-lg px-3 py-1.5 text-sm font-medium text-theme-muted transition-colors hover:bg-theme-panel hover:text-theme-foreground"
          >
            {t("common.close")}
          </button>
        </div>
        ...
        {/* Game preview */}
        <div className="border-b border-theme-glass px-5 py-3">
          <div className="flex items-center gap-3 rounded-xl border border-theme-glass bg-theme-panel px-4 py-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 text-sm">
                <span className="font-medium text-theme-foreground">{game.white}</span>
                <span className="text-theme-muted">{t("shareGameModal.vs")}</span>
                <span className="font-medium text-theme-foreground">{game.black}</span>
              </div>
              <div className="mt-0.5 flex items-center gap-2 text-xs text-theme-muted">
        ...
        >
          {filtered.length === 0 ? (
            <div className="rounded-xl border border-dashed border-theme-glass bg-theme-panel px-3 py-6 text-center text-xs text-theme-muted">
              {friends.length === 0
                  ...
                  <button
                    ...
                    className={`group flex w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition-all ${
                      selected
                        ...
                        : "border-transparent hover:border-theme-glass hover:bg-theme-panel"
                    }`}
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="truncate text-sm font-medium text-theme-foreground">
                        {f.name}
                      </span>
                      {f.rating != null && (
                    ...
                    {selected && (
                      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-brand-500 text-theme-on-accent">
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-2 border-t border-theme-glass px-5 py-4">
          <button
            onClick={onClose}
            className="inline-flex h-10 items-center justify-center rounded-lg border border-theme-glass bg-theme-panel px-4 text-sm font-medium text-theme-foreground transition-colors hover:bg-theme-panel"
          >
            {t("common.cancel")}
          </button>
          <button
            ...
            className={`inline-flex h-10 items-center justify-center gap-2 rounded-lg px-5 text-sm font-semibold text-theme-on-accent transition-all ${
              sent
            ...
            } disabled:cursor-not-allowed disabled:bg-theme-panel disabled:text-theme-muted disabled:shadow-none`}
          >
            {sent ? (
              <>
