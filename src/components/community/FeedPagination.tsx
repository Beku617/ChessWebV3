...
export function FeedPagination({
  ...
  const btnPage = (active: boolean) =>
    active
      ? `${btnBase} w-9 h-9 bg-teal-500 text-white shadow-[0_10px_24px_rgba(13,148,136,0.28)]`
      : `${btnBase} w-9 h-9 bg-white/[0.04] text-gray-300 hover:bg-white/[0.08] hover:text-teal-300`;
  ...
  return (
    <div className={`flex items-center gap-1.5 ${className}`}>
      <button
        ...
        className={`${btnBase} w-9 h-9 bg-white/[0.04] text-gray-400 hover:bg-white/[0.08] hover:text-teal-300`}
      >
        <ChevronLeft size={16} />
      </button>
