import { Trans } from "react-i18next";
import { ArrowLeft } from "lucide-react";

interface PuzzlePanelHeaderProps {
  onBack: () => void;
}

export function PuzzlePanelHeader({ onBack }: PuzzlePanelHeaderProps) {
  return (
    <div className="flex items-center justify-between px-3 py-2 border-b border-theme-glass">
      <button
        onClick={onBack}
        className="text-theme-muted hover:text-theme-foreground transition-colors"
      >
        <ArrowLeft size={20} />
      </button>
      <div className="flex items-center gap-2">
        <div className="w-7 h-7 bg-amber-600 rounded flex items-center justify-center">
          <span className="text-theme-on-accent font-bold text-xs"></span>
        </div>
        <span className="text-lg font-bold"><Trans>Puzzles</Trans></span>
      </div>
      <div className="w-5" /> {/* Spacer */}
    </div>
  );
}
