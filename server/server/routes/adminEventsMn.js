...
import { importEventsToMn } from "../services/adminContentImportService.js";
import {
...
function sanitizeEventMNPayload(rawBody = {}, { partial = false } = {}) {
  const data = {};
...
router.post("/import-from-en", async (req, res) => {
  try {
    const result = await importEventsToMn({
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const events = await EventMN.find(queryFilter)
        .sort({ priority: -1, createdAt: -1 })
        .skip((page - 1) * limit)
        .limit(limit)
        .lean();

      const total = await EventMN.countDocuments(queryFilter);
...
router.post(
  ...
  async (req, res) => {
    try {
      const { data, error } = sanitizeEventMNPayload(req.body, {
        partial: false,
      });
      if (error) {
      ...
      const event = await EventMN.create(data);
...
router.put("/:id", [...idParamValidation, validateRequest], async (req, res) => {
  try {
    ...
    const event = await EventMN.findByIdAndUpdate(
      req.params.id,
      updateDoc,
...
router.delete("/:id", [...idParamValidation, validateRequest], async (req, res) => {
  try {
    const event = await EventMN.findByIdAndDelete(req.params.id);
