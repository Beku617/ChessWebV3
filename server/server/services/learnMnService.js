...
async function findOrCreateProgress({
  ...
  let progress = await UserLearnProgressMN.findOne({
    userId: userObjectId,
    courseId: courseObjectId,
  });

  if (!progress) {
    progress = new UserLearnProgressMN({
      userId: userObjectId,
      courseId: courseObjectId,
...
async function getCourseCatalog({
  ...
  let courses = await LearnMN.find(courseFilter)
  ...
  const lessons = await LessonMN.find({
  ...
  const progressDocs = await UserLearnProgressMN.find({
...
async function getCourseBySlug({ userId, courseSlug }) {
  ...
  const progress = await UserLearnProgressMN.findOne({
    userId: asObjectId(userId),
...
async function resolveCourseLessonContext({ courseSlug, lessonSlug }) {
  ...
  const steps = await LessonStepMN.find({
...
async function searchLearnContent({ query }) {
  ...
  if (lessons.length > 0) {
    ...
    if (missingCourseIds.length > 0) {
      const relatedCourses = await LearnMN.find({
...
async function getCourseOrLessonByPairId({ pairId }) {
  ...
  let lessonView = null;
  if (lesson) {
    const lessonCourse = await LearnMN.findById(lesson.courseId)
