...
import {
  LEARN_CATEGORIES,
  LEARN_DIFFICULTIES,
} from "../models/LearnMN.js";
import { LEARN_STEP_VALIDATION_MODES } from "../models/LessonStepMN.js";
...
async function getLessonCountMap(courseIds = []) {
  ...
  const rows = await LessonMN.aggregate([
...
async function getStepCountMap(lessonIds = []) {
  ...
  const rows = await LessonStepMN.aggregate([
...
async function ensureUniqueCourseSlug(slug, excludeCourseId = null) {
  ...
  const existing = await LearnMN.findOne(query).select("_id").lean();
  if (existing) {
...
async function ensureUniqueLessonSlug({ courseId, slug, excludeLessonId = null }) {
  ...
  const existing = await LessonMN.findOne(query).select("_id").lean();
  if (existing) {
...
async function refreshCourseTotalLessons(courseId) {
  const totalLessons = await LessonMN.countDocuments({
    courseId: asObjectId(courseId, "course id"),
  });
  await LearnMN.updateOne(
...
async function fetchCourseOrThrow(courseId) {
  const course = await LearnMN.findById(asObjectId(courseId, "course id"));
  if (!course) {
...
async function fetchLessonOrThrow(lessonId) {
  const lesson = await LessonMN.findById(asObjectId(lessonId, "lesson id"));
  if (!lesson) {
...
async function fetchStepOrThrow(stepId) {
  const step = await LessonStepMN.findById(asObjectId(stepId, "step id"));
  if (!step) {
...
async function applyLessonOrder(courseId, orderedLessonIds) {
  ...
  const lessons = await LessonMN.find({ courseId: safeCourseId })
  ...
  if (tempOps.length) {
    await LessonMN.bulkWrite(tempOps);
    await LessonMN.bulkWrite(finalOps);
  }

  return LessonMN.find({ courseId: safeCourseId })
    .sort({ orderIndex: 1, createdAt: 1 })
    .lean();
}
...
async function applyStepOrder(lessonId, orderedStepIds) {
  ...
  const steps = await LessonStepMN.find({ lessonId: safeLessonId })
    .sort({ orderIndex: 1, createdAt: 1 })
    .lean();
  ...
  if (tempOps.length) {
    await LessonStepMN.bulkWrite(tempOps);
    await LessonStepMN.bulkWrite(finalOps);
  }

  return LessonStepMN.find({ lessonId: safeLessonId })
    .sort({ orderIndex: 1, createdAt: 1 })
    .lean();
}
...
async function listAdminCourses({
  ...
  const courses = await LearnMN.find(filter)
  ...
  const [lessonCounts, totalCourses, totalLessons, totalPublished] =
    await Promise.all([
      ...
      LearnMN.countDocuments({ isPublished: true }),
    ]);
...
async function createAdminCourse(payload = {}) {
  ...
  const created = await LearnMN.create({
    slug,
    title,
...
async function deleteAdminCourse(courseId) {
  const safeCourseId = asObjectId(courseId, "course id");
  const course = await LearnMN.findById(safeCourseId).lean();
  ...
  if (lessonIds.length > 0) {
    await LessonStepMN.deleteMany({ lessonId: { $in: lessonIds } });
  }
  await LessonMN.deleteMany({ courseId: safeCourseId });
  await UserLearnProgressMN.deleteMany({ courseId: safeCourseId });
  await LearnMN.deleteOne({ _id: safeCourseId });
...
async function listAdminLessons({
  ...
  const course = await LearnMN.findById(asObjectId(courseId, "course id")).lean();
  if (!course) throw new AdminLearnError("Course not found.", 404);
  ...
  const lessons = await LessonMN.find(filter)
    .sort({ orderIndex: 1, createdAt: 1 })
    .lean();
...
async function createAdminLesson(courseId, payload = {}) {
  ...
  const [maxOrder] = await LessonMN.find({ courseId: course._id })
    .sort({ orderIndex: -1 })
    .limit(1)
    .select("orderIndex")
    .lean();
  ...
  const lesson = await LessonMN.create({
    courseId: course._id,
    slug,
    title,
    subtitle: ensureString(payload.subtitle),
...
async function updateAdminLesson(lessonId, payload = {}) {
  ...
  if (targetOrderIndex !== undefined) {
    const allLessons = await LessonMN.find({ courseId: lesson.courseId })
      .sort({ orderIndex: 1, createdAt: 1 })
      .lean();
  ...
  const latest = await LessonMN.findById(lesson._id).lean();
  const [stepCountRow] = await LessonStepMN.aggregate([
...
async function deleteAdminLesson(lessonId) {
  ...
  await LessonStepMN.deleteMany({ lessonId: lesson._id });
  await LessonMN.deleteOne({ _id: lesson._id });
  const currentLessonIds = (
    await LessonMN.find({ courseId: lesson.courseId })
      .sort({ orderIndex: 1, createdAt: 1 })
      .select("_id")
      .lean()
  ...
  const fallbackLesson = await LessonMN.findOne({ courseId: lesson.courseId })
    .sort({ orderIndex: 1, createdAt: 1 })
    .select("_id")
    .lean();

  await UserLearnProgressMN.updateMany(
    { courseId: lesson.courseId },
    {
      $pull: {
  ...
  if (fallbackLesson?._id) {
    ...
    await UserLearnProgressMN.deleteMany({ courseId: lesson.courseId });
  }
...
async function listAdminSteps({ lessonId, search = "" }) {
  const lesson = await LessonMN.findById(asObjectId(lessonId, "lesson id")).lean();
  if (!lesson) throw new AdminLearnError("Lesson not found.", 404);
  const course = await LearnMN.findById(lesson.courseId).lean();
  if (!course) throw new AdminLearnError("Course not found.", 404);
  ...
  const steps = await LessonStepMN.find(filter)
    .sort({ orderIndex: 1, createdAt: 1 })
    .lean();
...
async function createAdminStep(lessonId, payload = {}) {
  ...
  const [maxOrder] = await LessonStepMN.find({ lessonId: lesson._id })
    .sort({ orderIndex: -1 })
    .limit(1)
    .select("orderIndex")
    .lean();
  ...
  const created = await LessonStepMN.create({
    lessonId: lesson._id,
    orderIndex: nextOrder,
...
async function updateAdminStep(stepId, payload = {}) {
  ...
  if (payload.orderIndex !== undefined) {
    const allSteps = await LessonStepMN.find({ lessonId: step.lessonId })
      .sort({ orderIndex: 1, createdAt: 1 })
      .lean();
  ...
  const latest = await LessonStepMN.findById(step._id).lean();
  return serializeStep(latest);
}

async function deleteAdminStep(stepId) {
  const step = await fetchStepOrThrow(stepId);
  await LessonStepMN.deleteOne({ _id: step._id });

  const remainingIds = (
    await LessonStepMN.find({ lessonId: step.lessonId })
      .sort({ orderIndex: 1, createdAt: 1 })
      .select("_id")
      .lean()
  ).map((entry) => toId(entry._id));
