import {
  ...
  updateAdminLesson,
  updateAdminStep,
} from "../services/adminLearnMnService.js";
import { importLearnToMn } from "../services/adminContentImportService.js";
...
function handleError(res, action, error) {
  ...
  console.error(`Admin learn mn ${action} error:`, error);
...
async function importFromEn(req, res) {
  try {
    const data = await importLearnToMn({
      sourceCourseIds: req.body?.courseIds || [],
    });
    res.json(data);
  } catch (error) {
    handleError(res, "import from en", error);
  }
}
